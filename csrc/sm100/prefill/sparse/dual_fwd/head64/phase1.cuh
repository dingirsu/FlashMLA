...
namespace sm100::dual_fwd::head64 {
...
KernelTemplate<D_QK>::sparse_attn_fwd_kernel_devfunc(const SparseAttnFwdParams &params, const TmaParams &tma_params) {
    ...
    const int q_pair_idx = blockIdx.x / 2;
    const int q_token_idx = 2*q_pair_idx + cta_idx;
    const int warp_idx = cutlass::canonical_warp_idx_sync();
    const int lane_idx = threadIdx.x % 32;
    const int topk_length = params.topk_length != nullptr ? __ldg(params.topk_length + q_pair_idx) : params.topk;
    ...
    int* gIndices = params.indices + q_pair_idx*params.stride_indices_s_q; // shared by the query-token pair
    ...
    if (warp_idx == 0) {
        if (elect_one_sync()) {
            // Each CTA loads one native [64, D_Q] query-token tile. The two
            // tiles are combined only by the 2-SM cooperative MMA.
            Tensor gQ = tma_params.tma_Q.get_tma_tensor(tma_params.shape_Q)(_, _, q_token_idx);
    ...
    if (warpgroup_idx == 0) {
        ...
        if (idx_in_warpgroup < 64) {
            int global_index = q_token_idx*(B_H/2) + idx_in_warpgroup;
        ...
        Tensor tma_gO = flat_divide(
            tma_params.tma_O.get_tma_tensor(tma_params.shape_O)(_, _, q_token_idx),
...
void run_fwd_phase1_kernel(const SparseAttnFwdParams& params) {
    ...
    KU_ASSERT(params.h_q == Kernel::H_Q);
    KU_ASSERT(params.s_q % 2 == 0);
    KU_ASSERT(params.d_qk == D_QK);

    const int num_q_pairs = params.s_q / 2;
